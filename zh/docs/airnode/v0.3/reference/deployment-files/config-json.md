...
(required) - The cloud provider that the node will be deployed at and its configuration.
...
The Airnode's HTTP gateway to test out endpoints without using the blockchain. Currently supported only by AWS cloud provider.
