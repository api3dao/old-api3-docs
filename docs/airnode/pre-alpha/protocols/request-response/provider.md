...
`providerAdmin` is an address that is authorized to update provider-related properties (e.g., endpoint authorizers). Airnode sets a `providerAdmin` while creating the provider record, and this is sourced from [config.json](../../airnode/specifications/config-json.md) under the name `providerAdminForRecordCreation`.
