...
[README](https://github.com/api3dao/airnode/tree/v0.6/packages/airnode-deployer/docker)
...
docker run -it --rm \
  ...
  api3/airnode-deployer:0.6.6 deploy
```
...
docker run -it --rm \
  --env-file aws.env \
  -v "$(pwd)/output:/app/output" \
  api3/airnode-deployer:0.6.6 remove -r output/receipt.json
```
