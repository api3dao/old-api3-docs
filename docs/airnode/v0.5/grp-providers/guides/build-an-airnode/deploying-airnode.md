...
around the
[deployer CLI](https://github.com/api3dao/airnode/tree/v0.5/packages/airnode-deployer).
...
docker run -it --rm \
  ...
  api3/airnode-deployer:0.5.2 deploy
```
...
docker run -it --rm \
  --env-file aws.env \
  -v "$(pwd)/output:/app/output" \
  api3/airnode-deployer:0.5.2 remove -r output/receipt.json
```
