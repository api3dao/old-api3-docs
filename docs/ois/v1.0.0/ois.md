...
[<InfoBtnBlue/>](/airnode/v0.6/grp-providers/guides/build-an-airnode/api-security.md)
...
in the
[API Security](../../airnode/v0.6/grp-providers/guides/build-an-airnode/api-security.md)
section of the _Build an Airnode_ guide and the
[Airnode Authentication](../../airnode/v0.6/concepts/airnode-auth.md) section of
...
[<InfoBtnBlue/>](/airnode/v0.6/grp-providers/guides/build-an-airnode/api-integration.md#fixedoperationparameters)
...
[<InfoBtnBlue/>](/airnode/v0.6/grp-providers/guides/build-an-airnode/api-integration.md#parameters)
...
(Optional) Defines the postpostprocessing code that can be used to modify the
API response from the request defined by this endpoint.
...
[preprocessing](ois.html#_5-9-preprocessingspecifications) and
[postprocessing](ois.html#_5-10-postprocessingspecifications).

The processing schema accepts an array of processing snippets which are chained.
The first snippet receives the endpoint parameters. The output of this snippet
is passed to the second snippet and so on.
...
- `environment` - Currently one of `Node 14` or `Node 14 async`. Both options
  ...
  version can use asynchronous code. The final `output` can be a promise and
  Airnode will use the resolved value.
- `value` - The processing code written as string.
- `timeoutMs` - The maximum timeout that this snippet can run. In case the
  timeout exceeded an error is thrown.
...
The processing snippet can use any Node.js builtin modules. Also the following
modules are available:

- `ethers` - You can use the [ethers](https://docs.ethers.io/v5/) library to use
  the common blockchain functions. Version `5.4.5` is used. _Note: the `ethers`
  object is imported via named imports, `import {ethers} from 'ethers`._
- `axios` - You can use the [axios](https://axios-http.com/) library to perform
  asynchronous web requests. Version `0.27.2` is used. _Note: The library is
  imported using a default import statemet, `import axios from 'axios';`_

::: warning Error handling and security

The processing code is specified by the Airnode operator and thus the processing
code can be trusted. It is the responsibility of the Airnode operator to ensure
the snippet code is valid.

Careful, when the processing code throws an error the API request is not
attempted.
