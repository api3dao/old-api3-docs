...
Follow the instructions for your Airnode to create your provider record using your master wallet, and it will send any unused ETH to the `airnodeAdminForRecordCreation` you have set in your `config.json`.
...
The receipt contains your [`airnodeId`](../../../reference/protocols/request-response/airnode.md#airnodeid), `airnodeIdShort` and `masterWalletAddress` that you will need to fund for it to create your provider record (if you have not already).
You will need to add your `airnodeIdShort` to your `config.json` to be able to redeploy your node with updated configurations.
